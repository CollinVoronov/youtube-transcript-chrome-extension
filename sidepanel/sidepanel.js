...
${transcriptText}`;
  ...
  // Copy to clipboard using fallback method (navigator.clipboard can fail in side panels)
  let copied = false;
  try {
  ...
  if (copied) {
    showToast('Prompt copied! Paste it in Claude (Cmd+V)', 'success');
    // Small delay so user sees the toast before tab switches
    setTimeout(() => {
      window.open('https://claude.ai/new', '_blank');
    }, 500);
  } else {
    showToast('Failed to copy prompt to clipboard');
  }
}
...
document.getElementById('insightsHeader').addEventListener('click', () => {
...
chrome.runtime.onMessage.addListener((message) => {
  if (message.type === 'VIDEO_UPDATED') {
    videoTitle.textContent = message.title || 'YouTube Video';
    insightsContainer.style.display = 'none';
